...
filter_length = 15
sonar0_buffer_filter = [0.8] * filter_length
sonar1_buffer_filter = [0.8] * filter_length
sonar2_buffer_filter = [0.8] * filter_length
sonar3_buffer_filter = [0.8] * filter_length
count_filter = 0
sum_data_sonar0 = 12
sum_data_sonar1 = 12
sum_data_sonar2 = 12
...
class BaseController:
    ...
    def poll(self):
        now = rospy.Time.now()
        global sonar0_buffer_filter
        global sonar1_buffer_filter
        global sonar2_buffer_filter
        global sonar3_buffer_filter
        global count_filter
        global sum_data_sonar0
        global sum_data_sonar1
        global sum_data_sonar2
        global sum_data_sonar3
        if now > self.t_next:
		...
		pcloud = PointCloud2()
                try:
                    ...
                    if (self.sonar_r3 < 3) :
                    	self.sonar_r3 = 80
                    sum_data_sonar0 = sum_data_sonar0 + self.sonar_r0/100.0 - sonar0_buffer_filter[count_filter]
                    sonar0_buffer_filter[count_filter] = self.sonar_r0/100.0
                    filter_out_r0 = sum_data_sonar0 / filter_length

                    sum_data_sonar1 = sum_data_sonar1 + self.sonar_r1/100.0 - sonar1_buffer_filter[count_filter]
                    sonar1_buffer_filter[count_filter] = self.sonar_r1/100.0
                    filter_out_r1 = sum_data_sonar1 / filter_length

                    sum_data_sonar2 = sum_data_sonar2 + self.sonar_r2/100.0 - sonar2_buffer_filter[count_filter]
                    sonar2_buffer_filter[count_filter] = self.sonar_r2/100.0
                    filter_out_r2 = sum_data_sonar2 / filter_length

                    sum_data_sonar3 = sum_data_sonar3 + self.sonar_r3/100.0 - sonar3_buffer_filter[count_filter]
                    sonar3_buffer_filter[count_filter] = self.sonar_r3/100.0
                    filter_out_r3 = sum_data_sonar3 / filter_length
                    ...
                    sonar0_range.range = filter_out_r0
                    self.sonar0_pub.publish(sonar0_range)
                    # sum_data_sonar0 = sum_data_sonar0 + sonar0_range.range - sonar0_buffer_filter[count_filter]
                    # sonar0_buffer_filter[count_filter] = sonar0_range.range
                    # filter_out = sum_data_sonar0/filter_length
                    
                    # if filter_out >= sonar0_range.max_range or filter_out == 0.0:
                    #     filter_out = sonar0_range.max_range
                    # if filter_out >= 0.5 or filter == 0.0:
                    #     self.sonar_cloud[0][0] = self.sonar0_offset_x + self.sonar_maxval * math.cos(self.sonar0_offset_yaw)
                    #     self.sonar_cloud[0][1] = self.sonar0_offset_y + self.sonar_maxval * math.sin(self.sonar0_offset_yaw)
                    # else:
                    #     self.sonar_cloud[0][0] = self.sonar0_offset_x + filter_out * math.cos(self.sonar0_offset_yaw)
                    #     self.sonar_cloud[0][0] = self.sonar0_offset_y + filter_out * math.sin(self.sonar0_offset_yaw)
                    

                    if sonar0_range.range>=sonar0_range.max_range or sonar0_range.range == 0.0:
                        sonar0_range.range = sonar0_range.max_range
                    self.sonar0_pub.publish(sonar0_range)
                    if sonar0_range.range>=0.45 or sonar0_range.range == 0.0:
                        self.sonar_cloud[0][0] = self.sonar0_offset_x + self.sonar_maxval * math.cos(self.sonar0_offset_yaw)
                        self.sonar_cloud[0][1] = self.sonar0_offset_y + self.sonar_maxval * math.sin(self.sonar0_offset_yaw)
                    else:
                    ...
                    sonar1_range.range = filter_out_r1
                    ...
                    if sonar1_range.range>=0.45 or sonar1_range.range == 0.0:
                        self.sonar_cloud[1][0] = self.sonar1_offset_x + self.sonar_maxval * math.cos(self.sonar1_offset_yaw)
                        self.sonar_cloud[1][1] = self.sonar1_offset_y + self.sonar_maxval * math.sin(self.sonar1_offset_yaw)
                    else:
                    ...
                    sonar2_range.range = filter_out_r2
                    ...
                    if sonar2_range.range>=0.45 or sonar2_range.range == 0.0:
                        self.sonar_cloud[2][0] = self.sonar2_offset_x + self.sonar_maxval * math.cos(self.sonar2_offset_yaw)
                        self.sonar_cloud[2][1] = self.sonar2_offset_y + self.sonar_maxval * math.sin(self.sonar2_offset_yaw)
                    else:
                    ...
                    sonar3_range.range = filter_out_r3
                    ...
                    if sonar3_range.range>=0.45 or sonar3_range.range == 0.0:
                        self.sonar_cloud[3][0] = self.sonar3_offset_x + self.sonar_maxval * math.cos(self.sonar3_offset_yaw)
                        self.sonar_cloud[3][1] = self.sonar3_offset_y + self.sonar_maxval * math.sin(self.sonar3_offset_yaw)
                    else:
